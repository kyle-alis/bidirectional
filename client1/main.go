...
func bulkUsers(c users.UsersClient) {
	...
	if err != nil {
		fmt.Errorf("Error when getting stream object: %v", err)
		return
	}
	...
	go func() {
		for {
			...
			if err != nil {
				fmt.Errorf("Error when receiving response: %v", err)
			}
...
func (c container) initUsers() []*users.User {
	...
	c.users = append(c.users, c.getUser("1", "Marisol", "Richardson", 29))
	c.users = append(c.users, c.getUser("1", "Mia", "Phill", 27))
	c.users = append(c.users, c.getUser("1", "Tomas", "Smith", 25))
	c.users = append(c.users, c.getUser("1", "Zian", "Heat", 28))
